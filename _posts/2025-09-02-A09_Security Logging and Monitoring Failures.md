...
2.  **로그 무결성 보장:** Part 3에서 확인된 삭제 위협에 대응하기 위해서 모든 로그는 생성되는 즉시 별도의 중앙 로그 관리 서버로 실시간 전송해야 합니다. 이 서버는 한 번 기록된 내용은 수정하거나 삭제할 수 없도록 `쓰기 전용` 설정하여 로그의 위변조를 원천적으로 막아야 합니다.
