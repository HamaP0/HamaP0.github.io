...
1.  다이어그램 도구를 실행한다.
2.  'User PC', 'Recursive DNS', 'Root DNS', 'TLD DNS (.com)', 'Authoritative DNS (google.com)' 5개의 박스를 그린다.
3.  User PC에서 Recursive DNS로 향하는 화살표부터 시작하여, Root -> TLD -> Authoritative 순으로 쿼리가 진행되는 과정을 번호와 함께 화살표로 그린다.
4.  Authoritative DNS에서 최종 IP 주소가 Recursive DNS를 거쳐 User PC로 반환되는 응답 과정을 화살표로 그린다.
5.  전체 흐름을 `Dns_query_flow.png`로 저장한다.