...
#### **5. 근본 원인 및 해결 방안: 설계 단계부터의 무결성 확보**
...
*   **해결 방안:**
1.  **소프트웨어 무결성 확보 (SRI 적용):** 외부에서 로드하는 모든 스크립트, 스타일시트 등 제3자 리소스에 대해 **Subresource Integrity(SRI)를 의무적으로 적용**해야 합니다. `integrity` 속성을 통해 브라우저가 직접 파일의 무결성을 검증하도록 강제하여 공급망 공격의 위험을 크게 줄일 수 있습니다.

2.  **데이터 무결성 확보 (안전한 데이터 포맷 사용):**
    *   **JSON과 같은 안전한 데이터 전용 포맷을 사용**하고, 임의의 객체를 생성할 수 있는 직렬화 포맷의 사용은 가급적 피해야 합니다.
    *   데이터를 수신할 때는 `json_decode()`처럼 코드 실행 가능성이 없는 안전한 파서(Parser)를 사용하고, 역직렬화를 꼭 사용해야 한다면 데이터의 유효성을 철저히 검증하고 디지털 서명을 통해 데이터의 무결성을 보장해야 합니다.
    *   궁극적으로, 이는 문제가 발생한 뒤에 고치는 대응적 통제가 아닌, **설계 단계에서부터 위험 요소를 배제하는 '안전한 설계(Secure by Design)' 원칙**을 적용하는 것이 가장 이상적인 해결책입니다.
