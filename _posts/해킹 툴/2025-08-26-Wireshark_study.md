...
#### **ICMP 패킷 분석 화면 스크린샷 제작**

1.  Wireshark를 실행하고 Attacker PC의 네트워크 인터페이스(예: `eth0`)에서 캡처를 시작합니다.
2.  터미널에서 `ping -c 1 192.9.200.11` 명령을 실행합니다.
3.  Wireshark에서 캡처를 중지합니다.
4.  상단 디스플레이 필터 창에 `icmp`를 입력하고 Enter 키를 누릅니다.
5.  패킷 목록 창에 나타난 두 개의 ICMP 패킷 중 첫 번째 `Request` 패킷을 클릭하여 선택합니다.
6.  이 상태에서 Wireshark 창 전체를 스크린샷으로 찍습니다.
7.  이미지 편집 프로그램을 사용하여 스크린샷을 엽니다.
8.  주요 영역에 사각형 박스와 텍스트로 설명을 추가합니다.
    *   **필터 입력창:** `icmp`가 입력된 부분에 박스를 치고 "디스플레이 필터" 라고 표시합니다.
    *   **패킷 목록:** `Request`와 `Reply`가 보이는 두 줄에 박스를 치고 "패킷 목록" 이라고 표시합니다.
    *   **패킷 상세:** 중간의 `Internet Control Message Protocol` 부분이 확장된 곳에 박스를 치고 "패킷 상세" 라고 표시합니다.
    *   **패킷 바이트:** 하단의 16진수 데이터 부분에 박스를 치고 "패킷 바이트" 라고 표시합니다.
9.  수정된 이미지를 저장하여 게시글에 삽입합니다.

#### **이미지 2 제작 (HTTP POST 패킷 상세 분석)**

1.  Wireshark 캡처를 실행한 상태에서, DVWA에 `admin` / `password`로 로그인합니다.
2.  캡처를 중지하고 필터에 `http.request.method == "POST"`를 입력합니다.
3.  `POST /login.php` 패킷을 선택하고, **패킷 상세** 창에서 `HTML Form URL Encoded` 항목을 확장합니다.
4.  `password: password` 필드가 평문으로 보이는 상태에서 Wireshark 창 전체를 스크린샷으로 찍습니다.
5.  필터 창, 선택된 패킷 라인, 그리고 `password: password`가 보이는 상세 정보 섹션에 각각 하이라이트를 적용하여 저장합니다.

#### **이미지 3 제작 (Follow TCP Stream 결과)**

1.  위 3번 상태의 `POST /login.php` 패킷을 마우스 오른쪽 클릭하고 `Follow > TCP Stream`을 선택합니다.
2.  요청/응답 내용이 보이는 새로운 팝업 창이 나타납니다.
3.  이 창에서 `username=admin&password=password` 라인 전체에 명확하게 하이라이트를 적용하여 스크린샷을 찍고 저장합니다.

#### **이미지 4 제작 (Wireshark TLS 설정)**

1.  Wireshark의 `Edit > Preferences > Protocols > TLS` 설정 창을 연다.
2.  `(Pre)-Master-Secret log filename` 필드에 키 로그 파일 경로가 입력된 상태에서 설정 창 전체를 스크린샷으로 찍는다.
3.  이미지 편집 프로그램을 사용하여 `(Pre)-Master-Secret log filename` 필드 부분에 명확한 하이라이트를 적용하여 저장한다.

#### **이미지 5 제작 (복호화된 HTTPS 패킷)**

1.  위 `6. 사용 예시 3`의 모든 단계를 수행한다.
2.  Wireshark의 패킷 목록 창에 `TLS`가 아닌 `HTTP`로 복호화되어 표시된 패킷들이 보이도록 한다.
3.  로그인 `POST` 요청 패킷을 선택한다.
4.  패킷 상세 창 하단에 `Decrypted TLS` 탭이 활성화되어 보이고, 그 내용에 평문 데이터가 나타난 상태에서 Wireshark 창 전체를 스크린샷으로 찍는다.
5.  이미지 편집 프로그램을 사용하여 스크린샷을 엽니다.
6.  세 부분에 하이라이트를 적용하여 복호화 과정을 명확히 보여준다.
    *   **패킷 목록:** `TLS`가 아닌 `HTTP`로 표시된 라인
    *   **패킷 상세:** `Decrypted TLS` 탭
    *   **패킷 바이트:** `Decrypted TLS` 탭을 선택했을 때 보이는 평문 데이터(예: `username=...`)
7.  수정된 이미지를 저장하여 게시글에 삽입한다.